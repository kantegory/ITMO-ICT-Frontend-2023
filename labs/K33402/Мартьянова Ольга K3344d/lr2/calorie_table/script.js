...
async function getCardHtml(product) {
    ...
    if (user) {
        ...
        if (favorites && favorites.includes(product.name)) {
            button += `<input class="btn btn-custom remove-btn" onclick="removeFromFavorites('${product.name}')" value="Remove from favorites">`;
        } else {
            button += `<input class="btn btn-custom add-btn" onclick="addToFavorites('${product.name}')" value="Add to favorites">`;
        }
    }
    return `
        <div class="card">
            <h5 class="card-header">${capitalizeFLetter(product.name)}</h5>
            <div class="card-body">
